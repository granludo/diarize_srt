...
def main():
    if len(sys.argv) != 5:
        print("Usage: python script.py <diarized_file> <str_file> <output_file> <speaker_name>")
        sys.exit(1)
    speaker_to_keep = sys.argv[4]
    ...
    with open(output_file, 'w') as file:
        ...
        for subtitle in subtitles:
            ...
            #    print(f"Sepaker found ={speaker}")
                ...
                print(f"ERROR:Missing speaker for subtitle: {subtitle}")
                exit(1)
    print(f"Lines kept: {lines_kept} for speaker {speaker_to_keep}")
    print(f"Lines removed: {len(subtitles)-lines_kept} for other speakers ")
    print("Done")        
